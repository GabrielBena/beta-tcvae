...
def estimate_entropies(qz_samples, qz_params, q_dist, n_samples=10000, weights=None):
    ...
    while k < S:
        ...
        entropies += -utils.logsumexp(logqz_i + weights, dim=0, keepdim=False).data.sum(
            1
        )
...
def mutual_info_metric_shapes(vae, shapes_dataset):
    ...
    for xs in dataset_loader:
        ...
        qz_params[n : n + batch_size] = (
            vae.encoder.forward(xs).view(batch_size, vae.z_dim, nparams).data
        )
        n += batch_size
    ...
    marginal_entropies = estimate_entropies(
        qz_samples.view(N, K).transpose(0, 1), qz_params.view(N, K, nparams), vae.q_dist
    )
...
if __name__ == "__main__":
    ...
    metric, marginal_entropies, cond_entropies = eval(
        "mutual_info_metric_" + cpargs.dataset
    )(vae, dataset)
    torch.save(
        {
            "metric": metric,
